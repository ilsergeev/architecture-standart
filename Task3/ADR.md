...
|NFR-5|Отклик по всем операциям измеряется миллисекундами (P1)|
...
|NFR-13|Сегментированный доступ к справочнику ставок: бэк-офис кредитов ведёт показатели риска, бэк-офис депозитов ведёт ставки; доступ к чужому сегменту закрыт (+R4)|
...
Данный подход был выбран, потому что MVP необходимо запустить в ближайшие 6 месяцев, и разбивать сразу весь монолит, ядро которого относится к подрядчику, нецелесообразно. Обновление ядра ИБ завязано на подрядчика, а этого мы избегаем.

Отдельные сервисы выбраны ещё и потому, что стек монолита (.NET Framework 4.5) технически не позволяет реализовать требуемое: он несовместим с Kafka, которую предписано использовать для очередей сообщений, и не даёт нормального горизонтального масштабирования. Требования по доступности 24/7 и 99,9% в текущем монолите тоже невыполнимы, он работает на одной группе серверов в одном ЦОД.

API Gateway нужен потому, что при реализации Strangler Fig надо уметь управлять маршрутизацией между монолитом ИБ и новым функционалом, оставаясь бесшовным для пользователя. Он же закрывает вопрос аутентификации: неизвестно, умеет ли монолит ИБ выдавать JWT-токены, а новым сервисам они нужны.
...
Выделить заявки в отдельный сервис продиктовано тем, что АБС и так высоконагруженная система и вводить сущность заявок в неё нельзя. Команда АБС прямо предупреждает, что онлайн-подача заявок может поставить под угрозу работоспособность банка. При таком решении в АБС попадает только финальная операция, то есть открытый депозит, а не весь черновой поток заявок, включая отклонённые и брошенные.
...
Новые сервисы депозитного контура пишутся на .NET (современная версия, не Framework 4.5) силами внутренней команды интернет-банка. В качестве СУБД для новых сервисов выбран PostgreSQL. Команда АБС делает свою часть: запись событий в outbox из PL-SQL и Kafka-адаптер. Доработка сайта тоже на команде ИБ.
...
Срок зависит от подрядчика, появляется внешняя зависимость в критичном проекте. Главное, что на .NET Framework 4.5 технически невозможно реализовать работу с Kafka и горизонтальное масштабирование.
...
- API Gateway становится критически важным узлом системы и требует резервирования: его отказ делает недоступным весь контур.
...
- Событие из АБС может не дойти, тогда депозит будет открыт, а уведомление клиенту не отправлено. Требуется мониторинг публикации событий.
...
- Внутренняя команда ИБ это 10 человек, они же сопровождают монолит. Новая разработка конкурирует за ресурс с текущей поддержкой.